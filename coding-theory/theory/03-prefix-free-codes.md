...
In this case, $`C_{2}`$ is instantaneously decodable. To see this, observe that decoding a message requires only counting the number of 1's that appear before a 0.
...
**Theorem:** A code is instantaneously decodable _if and only if_ it is prefix-free.
...
Thus the two notions describe _the same class of codes_, just from different perspectives:
...
This balance between _efficiency_ and _decodability_ is crucial. To formalize it, we need a way to describe _which sets of codeword lengths are even possible_ for a prefix-free code.

The Kraft&ndash;McMillan inequality provides exactly that: a simple condition that any collection of codeword lengths must satisfy to correspond to a prefix-free code.
