...
```math
\hat{M} = \arg\min_{M} \mathcal{L}_{\text{MDL}}(M)
```
...
Up until now, the notion of a description and its length has been intentionally left somewhat vague. In MDL theory, there are two traditional notions of descriptions:
...
- **Compression as Learning:** The best description of data is the program with the smallest length that reproduces the data, capturing all regularities while leaving minimal residual information.
...
where $`L(M)`$ is the number of bits required to describe the model itself, and $`L(D \mid M)`$ is the number of bits required to encode the data after accounting for the structure imposed by the model.
...
- **Learning is Compression:** The goal is to encode both the model and the data (or residuals) as efficiently as possible. The more the data can be compressed, the better it has been understood.
